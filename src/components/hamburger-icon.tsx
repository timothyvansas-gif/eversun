...
const LINE = "h-[1.5px] rounded-full origin-center";
...
export default function HamburgerIcon({ open = false, colorClassName = "bg-surface-page" }: { open?: boolean; colorClassName?: string }) {
  return (
    <>
      <m.span
        className={`w-6 ${LINE} ${colorClassName}`}
        animate={{ rotate: open ? 45 : 0, y: open ? 6.5 : 0 }}
        transition={T}
      />
      <m.span
        className={`w-4 ${LINE} ${colorClassName}`}
