...
export default function FaqList({
  ...
  return (
    <div className="border-t border-line/40">
      {FAQ.map((item, index) => (
        <details key={item.vraag} open={index === defaultOpenIndex} className="group border-b border-line/40">
          <summary className="flex cursor-pointer list-none items-start justify-between gap-6 py-4 transition-colors duration-150 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-accent [&::-webkit-details-marker]:hidden">
            ...
            <span
              aria-hidden
              className="mt-[3px] flex h-5 w-5 shrink-0 items-center justify-center text-muted transition-[rotate,color] duration-200 ease-out group-open:rotate-45 group-hover:text-accent"
            >
          ...
          <p className="pb-4 pr-8 font-sans text-[15px] leading-[24px] tracking-[-0.01em] text-muted">
            {item.antwoord}
          </p>
        </details>
      ))}
