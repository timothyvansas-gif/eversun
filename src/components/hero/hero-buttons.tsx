...
export default function HeroButtons({ onOpenAfspraak, onOpenPlanJeMoment }: { onOpenAfspraak: () => void; onOpenPlanJeMoment: () => void }) {
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
      ...
      <button
        className="group/cta w-[172px] px-0 sm:w-auto sm:px-10 min-h-[48px] sm:min-h-[56px] lg:min-h-[56px] font-sans font-semibold text-[14px] md:text-[16px] text-surface-page cursor-pointer active:scale-[0.98] transition-transform duration-200"
        onClick={() => {
      ...
      <button
        className="group/cta hidden lg:flex justify-center items-center rounded-full px-10 min-h-[56px] font-sans font-semibold text-[16px] text-white cursor-pointer border border-white/30 hover:border-white active:scale-[0.98] transition-[transform,border-color] duration-200"
        onClick={() => {
