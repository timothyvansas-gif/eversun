...
export default function HeroButtons({ onOpenAfspraak }: { onOpenAfspraak: () => void }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
      <button
        className="w-[60%] sm:w-[240px] min-h-[52px] sm:min-h-[56px] px-0 sm:px-5 font-sans font-medium text-[14px] sm:text-[15px] md:text-[16px] text-surface-page cursor-pointer active:scale-[0.98] transition-transform duration-200 relative overflow-hidden"
        onMouseEnter={(e) => {
