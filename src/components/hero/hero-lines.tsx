...
export default function HeroLines({ play }: { play: boolean }) {
  return (
    <m.svg
      ...
      className="absolute left-0 -bottom-[6px] md:-bottom-[3px] w-full"
    ...
    >
      <m.path
        ...
        className="[stroke-width:1] md:[stroke-width:1.75]"
      ...
      <m.path
        ...
        strokeLinecap="round"
        strokeLinejoin="round"
        className="[stroke-width:1] md:[stroke-width:1.75] translate-y-[-1px] md:translate-y-[2px]"
        initial={{ pathLength: 0, opacity: 0 }}
        animate={play ? { pathLength: 1, opacity: 1 } : { pathLength: 0, opacity: 0 }}
